...
// Click Shield: Logic to swap between menu and game focus
function toggleMenu(menuToShow) {
    ...
    if (menuToShow) {
        ...
        mobileControls.style.pointerEvents = "none"; // Disable game buttons while in menu
    } else {
        mobileControls.style.pointerEvents = "auto"; // Enable game buttons when playing
    }
}

document.getElementById("startBtn").onclick = (e) => { e.stopPropagation(); init(); };
document.getElementById("shopBtn").onclick = (e) => { e.stopPropagation(); toggleMenu(shopMenu); updateUI(); };
document.getElementById("closeShop").onclick = (e) => { e.stopPropagation(); toggleMenu(mainMenu); };
...
document.getElementById("settingsBtn").onclick = (e) => { e.stopPropagation(); toggleMenu(settingsModal); };
document.getElementById("backToMenu").onclick = (e) => { e.stopPropagation(); toggleMenu(mainMenu); };
...
function init() {
    ...
    generatePlatforms(); 
    gameActive = true;
    toggleMenu(null); // Clear all menus and focus game
    updateUI(); 
    update();
}
...
const setupBtn = (id, act) => {
    ...
    btn.ontouchend = (e) => { e.preventDefault(); keys[config[act]] = false; };
};
