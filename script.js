...
function draw() {
    ...
    if (maxHeight >= 1000) {
        ...
        for(let i=0; i<5; i++) { let lineY = cameraY + (i * 150) + (hue % 150); ctx.moveTo(0, lineY); ctx.lineTo(400, lineY + (windForce * 20)); }
        ctx.stroke();
    }
    platforms.forEach(p => {
    ...
    else if (playerColor === 'striped') { let grad = ctx.createLinearGradient(player.x, player.y, player.x+30, player.y+30); grad.addColorStop(0, "#333"); grad.addColorStop(0.5, "#fff"); grad.addColorStop(1, "#333"); ctx.fillStyle = grad; } 
    ...
    ctx.fillRect(player.x, player.y, 30, 30); ctx.shadowBlur = 0; ctx.globalAlpha = 1.0; ctx.restore();
}
...
function gameOver() {
    ...
    startBtn.innerText = "RETRY"; overlay.style.display = "block"; updateUI();
}

// --- CRITICAL MOBILE JUMP FIX ---
...
window.addEventListener("keydown", e => {
    ...
    if ((e.code === "Space" || e.code === "ArrowUp") && !player.jumping) { player.velY = JUMP_FORCE; player.jumping = true; }
    keys[e.code] = true;
});
...
const setupBtn = (id, key) => {
    const btn = document.getElementById(id);
    // TOUCHSTART: Only triggers ONCE per tap
    ...
    // TOUCHEND: Resets movement but NEVER resets gravity
    btn.addEventListener("touchend", (e) => {
        e.preventDefault();
        if (id !== "jumpBtn") keys[key] = false;
    }, { passive: false });
};
